...
Describe "Verifying integrity of module files" {
    Context "Validating PS1 Script files" {
        $allFiles = Get-ChildItem -Path $moduleRoot -Recurse -Filter "*.ps1" |
            Where-Object {$_.BaseName -notmatch "build|bootstrap|demo|publish"}
        ...
        foreach ($file in $allFiles)
        {
            ...
            It "[$name] Should have no trailing space" {
                $result = $file | Select-String "\s$" | Where-Object { $_.Line.Trim().Length -gt 0}
                if ($result)
                {
                    $result.LineNumber | Should -BeNullOrEmpty
                }
            }
            ...
            $t = $tokens | Where-Object TokenFlags -eq CommandName | Where-Object { Test-Path "alias:\$($_.Text)" } 
            if ($t)
            {
                $al = ($t.text | select -Unique) -join ' ,'
                It "[$name] [$al] Should not contain aliases" {
                    $t | Measure-Object | Select-Object -ExpandProperty Count | Should -Be 0
                }
    ...
    Context "Validating help.txt help files" {
        $allFiles = Get-ChildItem -Path $moduleRoot -Recurse -Filter "*.help.txt" 
