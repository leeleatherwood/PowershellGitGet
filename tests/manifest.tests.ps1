...
$root = ($here -replace "\\public|\\private|\\tests|\\module", "") 

$manifestdata = Import-PowerShellDataFile -Path "$root\src\Cisco.DS.Portal.psd1"

$ModuleName = "Cisco.DS.Portal"
$ModuleGuid = $manifestdata["guid"]



# Import our module to use InModuleScope
Get-Module -Name $ModuleName -All | Remove-Module -Force -ErrorAction Stop
$ManifestPath = (Resolve-Path -Path (Join-Path -Path $root\src -ChildPath $ModuleName`.psd1))
$changeLogPath = (Resolve-Path -Path (Join-Path -Path $root -ChildPath "CHANGELOG.MD"))
Import-Module $ManifestPath -Force


Describe "the Module Manifest - [$ModuleName`.psd1]" {
    ...
    It "has a valid guid" {
        $Script:Manifest.Guid | Should Be $ModuleGuid
    }
    ...
    It "has a valid HelpInfoUri" {
        ...
        { Invoke-WebRequest -Uri $Module.HelpInfoUri -UseBasicParsing} | Should Throw
    }
    ...
    foreach ($command in (Get-Command -Module $ModuleName))
    {
        if ($command.HelpUri)
        {
            It "$command has a valid HelpUri " {
                { Invoke-WebRequest -Uri $command.HelpUri -UseBasicParsing} | Should Not Throw
            }
        }
    }
}
